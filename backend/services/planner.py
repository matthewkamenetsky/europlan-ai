...
def create_trip(cities: list[str], trip_length: int, interests: list[str], existing_trip_id: int | None = None) -> tuple[int, str] | None:
    """Resolve cities, build prompt, insert pending DB row (or reuse existing), return (trip_id, prompt)."""
    with get_db() as db:
